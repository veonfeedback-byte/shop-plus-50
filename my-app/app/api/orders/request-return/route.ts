// app/api/orders/request-return/route.ts
...
export async function POST(req: Request) {
  ...
  const { data: order } = await supabaseAdmin
    .from('orders')
    .select('id,status,profile_id,completed_at,created_at')
    .eq('id', orderId)
    .maybeSingle();
  ...
  if (order.status !== 'complete') {
    return NextResponse.json({ error: 'Return available only after completion' }, { status: 400 });
  }

  const completedAt = order.completed_at ? new Date(order.completed_at).getTime() : 0;
  const within7 = completedAt && (Date.now() - completedAt) <= 7*24*60*60*1000;
  if (!within7) {
    return NextResponse.json({ error: 'Return window (7 days) passed' }, { status: 400 });
  }
